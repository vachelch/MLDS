...
def train_D(true_imgs, gen_imgs):
    ...
    loss_G = criterion(1 - gen_pred, Variable(torch.ones(batch_size).cuda()))
    loss_G.backward()
...
def train_G(gen_imgs):
    ...
    loss_G = criterion(1 - gen_pred, Variable(torch.zeros(batch_size).cuda()))
    loss_G.backward()
...
def gen_img(generator, batch_size=64, noise_dim = 100):
    noise = Variable(torch.rand(batch_size, noise_dim).cuda())
