...
def train_D(true_imgs, gen_imgs):
    ...
    loss_D = (-1 * (true_pred - gen_pred)).mean()
    loss_D.backward()
...
for i_iter in range(n_iter):
    ...
    if i_iter % print_every == 0:
        ...
        print('iter {}, G_loss: {:0.6f}, D_loss: {:0.6f}'.format(i_iter, loss_G.cpu().data[0], loss_D.cpu().data[0]))
        with open('log/log.txt', 'w+') as f:
            f.write('iter {}, G_loss: {:0.6f}, D_loss: {:0.6f}\n'.format(i_iter, loss_G.cpu().data[0], loss_D.cpu().data[0]))
    if i_iter % 10000 == 0:
