...
class Agent_DQN(Agent):
    ...
    def make_action(self,observation, test=True):
        ...
        if test and random.random()> FINAL_EPSILON:
            action = np.argmax(QValue)
        elif test:
            action = random.randrange(self.actions)
        ...
        for e in range(NUM_EPISODES):
            ...
            for s in range(MAX_NUM_STEPS):
                ...
                if done == True:
                    ...
                    train_history.append((e,step_count,total_reward,self.timeStep))
                    break
            if e % 100 == 0:
                test_history.append((test(self, self.env), e))

            if e % 1000 ==0:
                np.save("dqn_train_history.npy", np.array(train_history))
                np.save("dqn_test_history.npy", np.array(test_history))
